...
class RecordMultiCam(BaseSample):
    ...
    def setup_robot(self):
        ...
        self.sim_ctx.scene.add(Robot(prim_path="/Forklift", name="forklift", position = [-24.0, 7.0, 0.0]))
        articulation_view =  ArticulationView(prim_paths_expr = "/Forklift")
        self.forklift = self.sim_ctx.scene.get_object("forklift")
        self.forklift.get_articulation_controller()._articulation_view = articulation_view
        # self.forklift._articulation_view = articulation_view
        
            
    def drive_robot(self):
        self.forklift.apply_action(ArticulationAction(joint_positions=None,
                                                                            joint_efforts=None,
                                                                            joint_velocities = np.array([10, 10, 0, 0, 0, 10, 10])))
    ...
    def setup_people(self):
        ...
        self.agent_manager.spawn_character_by_idx(np.array([-2.6,18.4,0.0]), 0, 0)
    ...
    async def setup_post_load(self):
        self.target_fps = 30
        self.duration = 30 # in second
        self.resolution = (1280, 720)
        self.save_video_path = "/home/user/isaac_save/export_videos/test3"
        ...
        camera_list = ["/World/Cameras/Camera", "/World/Cameras/Camera_01"]
        ...
        for camera_path in camera_list:
            ...
            camera.initialize()
            print(camera_path, camera.get_intrinsics_matrix())
